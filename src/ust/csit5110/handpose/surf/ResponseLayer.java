	...
	private void buildResponseLayer(IntegralImage img){
		...
		for(int r, c, ar = 0, index = 0; ar < mHeight; ++ar) {
			for(int ac = 0; ac < mWidth; ++ac, index++){ 
			      ...
			      Dxx = IntegralImage.BoxIntegral(img, r - l + 1, c - b, 2*l - 1, w)
			          - IntegralImage.BoxIntegral(img, r - l + 1, c - l / 2, 2*l - 1, l)*3;
			      Dyy = IntegralImage.BoxIntegral(img, r - b, c - l + 1, w, 2*l - 1)
			          - IntegralImage.BoxIntegral(img, r - l / 2, c - l + 1, l, 2*l - 1)*3;
			      Dxy = + IntegralImage.BoxIntegral(img, r - l, c + 1, l, l)
			            + IntegralImage.BoxIntegral(img, r + 1, c - l, l, l)
			            - IntegralImage.BoxIntegral(img, r - l, c - l, l, l)
			            - IntegralImage.BoxIntegral(img, r + 1, c + 1, l, l);
