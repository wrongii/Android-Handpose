...
/**
 * A class to calculate the upright or free oriented interest points 
 * of an image lazily (will not calculate until getPoints() is called)
 */
public class Surf implements Serializable {
	private static final long serialVersionUID = 1L;
	
	private static final int HESSIAN_OCTAVES = 3;
	...
	private transient Bitmap mOriginalImage;
	...
	/**
	 * Single static entry point to do image comparison
	 * @param tmpBm
	 * @return
	 */
	public static int DoSurfCompare(Bitmap tmpBm, Surf template){
    	float ratio = Float.parseFloat(DataWareHouse.Instance().GetAttribute("CompareWidth")) 
    			/ (tmpBm.getWidth()==0?1F:tmpBm.getWidth());
    	tmpBm = ImageProcessing.resizeBitmap(tmpBm, ratio);
    	tmpBm = ImageProcessing.ExtractSkinColor(tmpBm);
    	Surf surf = new Surf(tmpBm);
		Map<SURFInterestPoint,SURFInterestPoint> matchingPoints;
		matchingPoints = surf.getMatchingPoints(template, true);
		return matchingPoints.size();
	}
	
	private IntegralImage mIntegralImage;
	
	public Surf(Bitmap image){
		this(image,HESSIAN_BALANCE_VALUE,HESSIAN_THRESHOLD,HESSIAN_OCTAVES);
	}
	
	public Surf(Bitmap image, float balanceValue, float threshold, int octaves){
	...
	private float haarX(int row, int column, int s){
		return IntegralImage.BoxIntegral(mIntegralImage, row-s/2, column, s, s/2)
			-1 * IntegralImage.BoxIntegral(mIntegralImage, row-s/2, column-s/2, s, s/2);
	}

	//TODO: Why the multiply by 1?
	private float haarY(int row, int column, int s){
		return IntegralImage.BoxIntegral(mIntegralImage, row, column-s/2, s/2, s)
			-1 * IntegralImage.BoxIntegral(mIntegralImage, row-s/2, column-s/2, s/2, s);
	}
